...
import { AlertCircle, Clock, Vote, Target, ArrowRight, Calendar } from 'lucide-react';
...
function ActionCard({ action }: { action: PendingAction }) {
  ...
  return (
    <Card className={`border-l-4 ${urgencyColor} hover:shadow-md transition-shadow`}>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-lg">
                {isSubmissionVote ? action.title : `${action.submission?.title} - ${action.title}`}
              </CardTitle>
              {(isCritical || isUrgent) && (
            ...
            <CardDescription className="flex items-center gap-4">
              <span>by {(isSubmissionVote ? action.submitter?.name : action.submission?.submitter?.name) || 'Anonymous'}</span>
         ...
         {(action.committee || action.submission?.committee) && (
           <CommitteeBadge 
             committee={{
               ...
               description: (action.committee || action.submission?.committee!)?.description || undefined,
               logoUrl: (action.committee || action.submission?.committee!)?.logoUrl || undefined,
...
export function PendingActionsPanel({ submissionsNeedingVote, milestonesNeedingReview }: PendingActionsPanelProps) {
  const totalActions = submissionsNeedingVote.length + milestonesNeedingReview.length;
      ...
      <Card className="border-l-4 border-l-orange-500 bg-orange-50/50">
        <CardHeader>
          ...
          <CardDescription className="text-orange-700">
            The following submissions and milestones require your immediate review and approval.
          </CardDescription>
        </CardHeader>
      </Card>
