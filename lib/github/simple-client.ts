...
function createHeaders(): HeadersInit {
  ...
  const token = process.env.NEXT_PUBLIC_GITHUB_TOKEN || process.env.GITHUB_PERSONAL_ACCESS_TOKEN;
  if (token) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        ...
        console.warn('[github-simple]: Rate limit exceeded. Remaining:', rateLimitRemaining);
        throw new Error('GitHub API rate limit exceeded. Please add GITHUB_TOKEN environment variable for higher limits.');
      }
...
export async function getRepositoryCommits(
  ...
  try {
    console.log('[github-simple]: Fetching commits from repository', { repoUrl, options });
    ...
    console.error('[github-simple]: Error fetching repository commits', { repoUrl, error });
    return null;
  }
}
...
export async function getCommitsSince(
  ...
  try {
    console.log('[github-simple]: Fetching commits since SHA', { repoUrl, sinceCommitSha, options });
    ...
    if (!refCommit) {
      console.error('[github-simple]: Could not find reference commit', { sinceCommitSha });
      return null;
    }
    ...
    console.error('[github-simple]: Error fetching commits since SHA', { repoUrl, sinceCommitSha, error });
    return null;
  }
}
...
export async function getCommitDetails(
  ...
  try {
    console.log('[github-simple]: Fetching commit details', { repoUrl, commitSha });
    ...
    console.error('[github-simple]: Error fetching commit details', { repoUrl, commitSha, error });
    return null;
  }
}
...
export async function getRepositoryInfo(repoUrl: string): Promise<{
  ...
  try {
    ...
    console.error('[github-simple]: Error fetching repository info', { repoUrl, error });
    return null;
  }
} 