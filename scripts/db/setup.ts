...
export async function printSystemOverview(): Promise<void> {
  try {
    ...
    const committees = await db
      .select()
      .from(groups)
      .where(eq(groups.type, 'committee'))
    ...
    if (teams.length > 0) {
      ...
      for (const team of teams.slice(0, 5)) {
        ...
        console.log(
          `   • ${team.name}: ${memberCount.length} members, ${submissionCount.length} submissions`
        )
      }
      if (teams.length > 5) {
...
export async function validateDataIntegrity(): Promise<boolean> {
  try {
    ...
    if (orphanedMemberships.length > 0) {
      console.warn(
        `⚠️  Found ${orphanedMemberships.length} orphaned group memberships`
      )
      isValid = false
    }
    ...
    if (orphanedSubmissions.length > 0) {
      console.warn(
        `⚠️  Found ${orphanedSubmissions.length} submissions with invalid reviewer group references`
      )
      isValid = false
    }
    ...
    if (usersWithoutGroups.length > 0) {
      console.warn(
        `⚠️  Found ${usersWithoutGroups.length} users without primary groups`
      )
      isValid = false
    }
...
export async function cleanupOrphanedData(): Promise<void> {
  try {
    ...
    if (orphanedDiscussions.length > 0) {
      console.log(
        `🗑️  Removing ${orphanedDiscussions.length} orphaned discussions`
      )
    ...
    if (orphanedNotifications.length > 0) {
      console.log(
        `🗑️  Removing ${orphanedNotifications.length} orphaned notifications`
      )
      // Implementation would go here
    }
...
export async function initializeDatabase(): Promise<void> {
  try {
    ...
    if (!integrityValid) {
      console.warn(
        '⚠️  Data integrity issues detected - consider running cleanup'
      )
    }
...
export async function quickSetup(): Promise<void> {
  try {
    ...
    // Add users to groups
    await addUserToGroup(reviewer.id, testCommittee.id, 'admin', [
      'manage_members',
      'approve_submissions',
    ])
    await addUserToGroup(teamMember.id, testTeam.id, 'admin', [
      'manage_team',
      'submit_applications',
    ])
    ...
    await db
      .update(users)
      .set({ primaryGroupId: testCommittee.id })
