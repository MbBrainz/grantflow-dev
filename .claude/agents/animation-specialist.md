...
>(({ className, animation = 'pulse', loading, children, ...props }, ref) => {
  ...
  return (
    <motion.div whileHover={animations[animation]} whileTap={{ scale: 0.95 }}>
      <Button
        ref={ref}
        className={cn(
...
export function ShimmerSkeleton({ className }: { className?: string }) {
  return (
    <motion.div
      className={cn(
        'from-muted via-muted-foreground/10 to-muted bg-gradient-to-r bg-[length:200px_100%] bg-no-repeat',
        className
      )}
...
<button
  class="rounded-md bg-blue-500 px-4 py-2 text-white transition-all duration-200 ease-in-out hover:scale-105 hover:bg-blue-600 hover:shadow-lg focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:outline-none active:scale-95"
>
  Animated Button
</button>
...
<div
  class="transform transition-transform duration-300 ease-out group-hover:translate-x-2 hover:scale-110 hover:rotate-3"
>
  Interactive Element
</div>
```
...
>
  ...
  <div
    class="absolute right-4 bottom-4 h-8 w-8 scale-0 transform rounded-full bg-white transition-all delay-150 duration-300 group-hover:scale-100"
  >
    →
  </div>
</div>
