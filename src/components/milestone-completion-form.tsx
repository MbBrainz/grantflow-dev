...
import { completeMilestone, validateTransactionHash } from '@/app/(dashboard)/dashboard/submissions/milestone-actions';
...
export function MilestoneCompletionForm({
  ...
  const validateForm = async () => {
    ...
    if (formData.transactionHash && formData.blockExplorerUrl) {
      const validation = await validateTransactionHash(formData.transactionHash, formData.blockExplorerUrl);
      if (!validation.valid) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.walletFrom) formDataObj.append('walletFrom', formData.walletFrom);
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            Mark "{milestoneTitle}" as completed by providing the multisig transaction details.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            {validationErrors.transactionHash && (
              <p className="text-sm text-red-500 mt-1">{validationErrors.transactionHash}</p>
            )}
          </div>
          ...
          <div>
            ...
            <Input
              ...
              className={validationErrors.blockExplorerUrl ? 'border-red-500' : ''}
              disabled={isSubmitting}
            />
            {validationErrors.blockExplorerUrl && (
              <p className="text-sm text-red-500 mt-1">{validationErrors.blockExplorerUrl}</p>
            )}
            <p className="text-xs text-gray-500 mt-1">
              Link to the transaction on a block explorer (e.g., Etherscan, Polygonscan)
            </p>
          </div>
        ...
        <div className="text-xs text-gray-500 bg-gray-50 p-3 rounded">
          ...
          The transaction hash must be from a successful multisig payment to the grantee.
        </div>
      </div>
    </Card>
  );
